...
const BROWSER_CONSTANTS_SCHEMA = {
    ...
    conversation: {
        ...
        items: {
            HISTORY_MODE: {
                label: '历史消息处理模式 (history_mode)',
                desc: '多轮对话处理模式：auto（智能续聊，页面已有对话只发新增轮次并跳过新建对话）、full（全量重放，总是填入完整历史）、last（单轮直发，始终仅发最后一轮消息）。',
                type: 'select',
                options: [
                    { value: 'auto', label: 'auto - 智能续聊（推荐，自动增量发送并跳过新建）' },
                    { value: 'full', label: 'full - 全量重放（每次发送全部历史并新建对话）' },
                    { value: 'last', label: 'last - 单轮直发（始终仅发送最后一轮消息）' }
                ],
                default: 'auto'
            },
...
const ENV_CONFIG_SCHEMA = {
    service: {
        ...
        items: {
            ...
            HISTORY_MODE: {
                label: '历史消息处理模式 (history_mode)',
                desc: '多轮对话模式：auto（智能续聊，优先增量发送并跳过新建对话）、full（全量重放，总是填入完整历史）、last（单轮直发，始终仅发最后一轮消息）',
                type: 'select',
                options: [
                    { value: 'auto', label: 'auto - 智能续聊（推荐）' },
                    { value: 'full', label: 'full - 全量重放' },
                    { value: 'last', label: 'last - 单轮直发' }
                ],
                default: 'auto'
            }
        }
