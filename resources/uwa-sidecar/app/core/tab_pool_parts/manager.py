...
class TabPoolManager:
    ...
    def _try_acquire_session_for_request(
        ...
        *,
        ...
        if turn_hint:
            aff_session = plugin_hooks.call(
                "find_affinity_session",
                sessions=sessions,
                turn_hint=turn_hint,
                default=None,
            )
            if aff_session is not None and aff_session.is_healthy(allow_live_check=False):
                if not self._should_defer_to_command(aff_session, task_id):
                    if aff_session.acquire(task_id):
                        logger.debug(f"[{aff_session.id}] 会话亲和命中: turn_hint 对齐成功")
                        return aff_session
        mode = self._normalize_allocation_mode(allocation_mode or self.allocation_mode)
        if mode == "first_idle":
            attempted_ids = set()
    ...
    async def acquire_async(self, task_id: str, timeout: float = None, turn_hint = None) -> Optional[TabSession]:
        return await self._run_cancellable_acquire(
            lambda: self.acquire(task_id, timeout, turn_hint=turn_hint),
            task_id,
        )
    ...
    def acquire(self, task_id: str, timeout: float = None, turn_hint = None) -> Optional[TabSession]:
