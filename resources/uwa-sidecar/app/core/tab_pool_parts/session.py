...
def _conversation_id_from_url(url: str) -> str:
    """从 URL 路径提取会话 ID（供 history_mode 插件记录会话指纹；逻辑与插件侧一致）。"""
    from urllib.parse import urlsplit
    raw = str(url or "").strip()
    if not raw:
        return ""
    try:
        return urlsplit(raw).path.rstrip("/")
    except ValueError:
        return raw
...
class TabSession:
    ...
    turn_keys: tuple[str, ...] = field(default_factory=tuple)
    last_typed: str = ""
    conversation_id: str = ""
    ...
    def remember_turn(
        ...
        url: str,
        typed: str,
        turn_keys: Optional[tuple[str, ...]] = None,
        history_mode: Optional[str] = None,
    ) -> None:
        """记录本轮对话指纹、URL 与已完成轮次，便于后续轮次续聊。"""
        with self._lock:
            self.turns += 1
            self.conversation_len = int(length or 0)
            self.conversation_url = str(url or "")
            self.conversation_id = _conversation_id_from_url(url)
            self.last_typed = str(typed or "")
            self.turn_keys = tuple(turn_keys or ())
            self.conversation_key = str(key or "")
            self.history_mode = history_mode
            self.last_conversation_activity_at = time.time()
            if url:
                self._refresh_current_domain(url)
    ...
    def get_info(
        ...
        *,
        ...
        with self._lock:
            ...
            turns = self.turns
            conversation_len = self.conversation_len
            conversation_id_val = self.conversation_id
            history_mode_val = self.history_mode
