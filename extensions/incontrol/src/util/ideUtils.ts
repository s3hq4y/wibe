...
export class VsCodeIdeUtils {
  ...
  async getUserSecret(key: string) {
    ...
    secret = await vscode.window.showInputBox({
      prompt: `Either enter secret for ${key} or press enter to skip this secret.`,
      password: true,
    });
  ...
  async getDebugLocals(threadIndex = 0): Promise<string> {
    ...
    if (!session) {
      vscode.window.showWarningMessage(
        t("No active debug session found, therefore no debug context will be provided for the llm.")
      );
      return "";
    }
