...
describe("NextEditWindowManager", () => {
  ...
  describe("Preview rendering", () => {
    /** The setDecorations call that carried the preview rows (if any). */
    const previewRows = (): any[] | undefined => {
      const calls = (mockEditor.setDecorations as MockedFunction<any>).mock
        .calls as any[][];
      const call = calls.find(
        (args) =>
          Array.isArray(args[1]) &&
          args[1].length > 0 &&
          args[1][0].renderOptions,
      );
      return call ? (call[1] as any[]) : undefined;
    };

    it("should render one native decoration row per predicted line", async () => {
      await manager.setupNextEditWindowManager(mockContext);
      ...
      // One decoration type for the preview block (plus one for deletions).
      const createDecoration = mockVscode.window
        .createTextEditorDecorationType as MockedFunction<any>;
      expect(createDecoration).toHaveBeenCalledWith(
        expect.objectContaining({ rangeBehavior: 1 }),
      );
      ...
      // Anchored at the end of editor lines 0, 1, 2 (region start + row).
      rows!.forEach((row, i) => {
        expect(row.range.start.line).toBe(i);
        expect(row.range.start.character).toBe(`line ${i} content`.length);
        expect(row.renderOptions.after.contentText).toContain(`new ${"abc"[i]}`);
        expect(row.hoverMessage).toBeDefined();
      });

      // Rows are padded to a common width so they form a block.
      const widths = new Set(
        rows!.map((row) => row.renderOptions.after.contentText.length),
      );
      expect(widths.size).toBe(1);

      // Colours come from the theme, not from a bundled highlighter.
      expect(rows![0].renderOptions.after.color).toBeInstanceOf(
        mockVscode.ThemeColor,
      );
      expect(rows![0].renderOptions.after.backgroundColor).toBeInstanceOf(
        mockVscode.ThemeColor,
      );
    });

    it("should fold rows past the end of the document into the last row", async () => {
      await manager.setupNextEditWindowManager(mockContext);
      ...
      expect(rows[1].renderOptions.after.contentText).toContain("two");
      expect(rows[1].renderOptions.after.contentText).toContain("three");
      expect(rows[1].renderOptions.after.contentText).toContain("four");
    });

    it("should expand tabs using the editor tab size", async () => {
      await manager.setupNextEditWindowManager(mockContext);
      mockEditor.options = { tabSize: 2 };
      ...
      const text = previewRows()![0].renderOptions.after.contentText as string;
