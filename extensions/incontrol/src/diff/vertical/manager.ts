...
export class VerticalDiffManager {
  ...
  // Listener for user doc changes is disabled during updates to the text document by incontrol
  ...
  async acceptRejectVerticalDiffBlock(
    ...
    // Disable listening to file changes while incontrol makes changes
    this.disableDocumentChangeListener();
  ...
  async streamDiffLines(
    ...
    vscode.commands.executeCommand("setContext", "incontrol.diffVisible", true);
  ...
  async streamEdit({
    ...
    if (existingHandler) {
      ...
      // but extra delta was added for each line generated by incontrol
      // Before adding this back, we need to distinguish between human and incontrol
    ...
    try {
      ...
      if (abortController.signal.aborted) {
        void vscode.commands.executeCommand("incontrol.rejectDiff");
      }
