      ...
      .filter(
        (s) =>
          ...
          (s.filepath !== currentFilepath &&
            // Exclude incontrol's own output as it makes it super-hard for users to test the autocomplete feature
            // while looking at the prompts in incontrol's output
            !s.filepath.startsWith(
              "output:extension-output-incontrol.incontrol",
            )),
      )
      .sort((a, b) => b.timestamp - a.timestamp)
