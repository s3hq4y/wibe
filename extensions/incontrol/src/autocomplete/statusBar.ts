...
const statusBarItemText = (
  ...
  // EXTENSION_NAME is the brand shown in the status bar, so a rename in
  // core/util/constants.ts is enough - no literal brand word here anymore.
  const brand = EXTENSION_NAME;

  if (error) {
    return `$(alert) ${brand} (config error)`;
  }
  ...
  switch (status) {
    case undefined:
      if (loading) {
        text = `$(loading~spin) ${brand}`;
      } else {
        text = brand;
      }
      break;
    case StatusBarStatus.Disabled:
      text = `$(circle-slash) ${brand}`;
      break;
    case StatusBarStatus.Enabled:
      text = `$(check) ${brand}`;
      break;
    case StatusBarStatus.Paused:
      text = `$(debug-pause) ${brand}`;
      break;
    default:
      text = brand;
  }
...
export function setupStatusBar(
  ...
  statusBarItem.command = "incontrol.openTabAutocompleteConfigMenu";
  ...
  if (!configListenerRegistered) {
    ...
    vscode.workspace.onDidChangeConfiguration((event) => {
      if (event.affectsConfiguration(INCONTROL_WORKSPACE_KEY)) {
        const enabled = getIncontrolWorkspaceConfig().get<boolean>(
