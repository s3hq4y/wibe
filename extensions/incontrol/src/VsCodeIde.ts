...
import { requireRipgrep } from "./util/ripgrep";
...
class VsCodeIde implements IDE {
  ...
  async isTelemetryEnabled(): Promise<boolean> {
    ...
    const incontrolEnabled: boolean =
      (await vscode.workspace
        .getConfiguration(EXTENSION_NAME)
        .get("telemetryEnabled")) ?? true;
    return globalEnabled && incontrolEnabled;
  }
  ...
  runRipgrepQuery(dirUri: string, args: string[]) {
    ...
    // No ripgrep is bundled with the extension: reuse VS Code's own copy (or
    // one on PATH). Throws RipgrepNotFoundError when neither exists.
    const rgPath = requireRipgrep();
    const p = child_process.spawn(rgPath, args, {
      cwd: relativeDir,
    });
    let output = "";
  ...
  async getSearchResults(query: string, maxResults?: number): Promise<string> {
    if (vscode.env.remoteName) {
      throw new Error("@search is not supported in remote workspaces");
    }
    const results: string[] = [];
  ...
  private getIdeSettingsSync(): IdeSettings {
    ...
    const ideSettings: IdeSettings = {
      ...
      incontrolTestEnvironment: "production",
    };
    return ideSettings;
  }
