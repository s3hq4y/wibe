...
import { IncontrolCompletionProvider } from "../autocomplete/completionProvider";
import {
...
export class VsCodeExtension {
  ...
  private consoleView: IncontrolConsoleWebviewViewProvider;
  private sidebar: IncontrolGUIWebviewViewProvider;
  ...
  private completionProvider: IncontrolCompletionProvider;
  ...
  private async updateNextEditState(
    ...
    const { config: incontrolConfig } = await this.configHandler.loadConfig();
    const autocompleteModel = incontrolConfig?.selectedModelByRole.autocomplete;
    ...
    if (
      ...
      !modelSupportsNext &&
      !isNextEditTest() &&
      process.env.INCONTROL_E2E_NON_NEXT_EDIT_TEST === "true"
    ) {
      vscode.window
        .showWarningMessage(
          `The current autocomplete model (${autocompleteModel?.title || "unknown"}) does not support Next Edit.`,
          t("Disable Next Edit"),
          t("Select different model"),
        )
        .then((selection) => {
          if (selection === t("Disable Next Edit")) {
            vscodeConfig.update(
              "enableNextEdit",
              false,
              vscode.ConfigurationTarget.Global,
            );
          } else if (selection === t("Select different model")) {
            vscode.commands.executeCommand(
              "incontrol.openTabAutocompleteConfigMenu",
            );
          }
        });
    }
  ...
  constructor(context: vscode.ExtensionContext) {
    ...
    this.sidebar = new IncontrolGUIWebviewViewProvider(
      this.windowId,
    ...
    this.completionProvider = new IncontrolCompletionProvider(
      this.configHandler,
      this.ide,
      this.sidebar.webviewProtocol,
      usingFullFileDiff,
    ...
    // LLM Log view
    this.consoleView = new IncontrolConsoleWebviewViewProvider(
      this.windowId,
      this.extensionContext,
      this.core.llmLogger,
    );
    ...
    context.subscriptions.push(
      vscode.window.registerWebviewViewProvider(
        "incontrol.incontrolConsoleView",
        this.consoleView,
      ),
    );
    ...
    const globalRulesDir = path.join(getIncontrolGlobalPath(), "rules");
    ...
    // TODO merge this and re-enable https://github.com/s3hq4y/incontrol/pull/8364
    ...
    context.subscriptions.push(
      vscode.workspace.registerTextDocumentContentProvider(
        VsCodeExtension.incontrolVirtualDocumentScheme,
  ...
  static incontrolVirtualDocumentScheme = EXTENSION_NAME;
