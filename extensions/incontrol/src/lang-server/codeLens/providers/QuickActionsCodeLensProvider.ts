...
export function toggleQuickActions() {
  ...
  getIncontrolWorkspaceConfig().update(ENABLE_QUICK_ACTIONS_KEY, curStatus);
}

export function quickActionsEnabledStatus() {
  return getIncontrolWorkspaceConfig().get<boolean>(ENABLE_QUICK_ACTIONS_KEY);
}
...
export class QuickActionsCodeLensProvider implements vscode.CodeLensProvider {
  ...
  getCustomCommands(
    ...
    return quickActionConfigs.map(({ title, prompt, sendToChat }) => {
      return sendToChat
        ? {
            title,
            command: "incontrol.customQuickActionSendToChat",
            arguments: [prompt, range],
          }
        : {
            title,
            command: "incontrol.customQuickActionStreamInlineEdit",
            arguments: [prompt, range],
          };
    });
  }
  ...
  getDefaultCommand(range: vscode.Range): vscode.Command[] {
    const quickEdit: vscode.Command = {
      command: "incontrol.defaultQuickAction",
      title: "incontrol",
