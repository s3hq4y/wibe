...
import llamaTokenizerModule from "./llamaTokenizer.js";

// The main-thread tokenizer is shipped two different ways, and the production
// build externalizes it as a CommonJS file (esbuild redirects
// `./llamaTokenizer.js` to `./llamaTokenizer.cjs` in `scripts/esbuild.js`).
// That external CJS module is produced from the ESM source by esbuild, so its
// `module.exports` is the transpiled ESM namespace ({ __esModule, default })
// rather than the `LlamaTokenizer` instance. With Node's interop the default
// import therefore resolves to that namespace object, and calling `.encode()`
// on it throws "X.default.encode is not a function". Dev/watch builds keep the
// tokenizer inlined as ESM, where the default import *is* the instance.
//
// Resolve whichever shape we actually received so ./encode() and ./decode()
// always exist, regardless of how the tokenizer module was packaged.
interface TokenizerLike {
  encode(text: string, ...rest: unknown[]): number[];
  decode(tokens: number[], ...rest: unknown[]): string;
}

function resolveLlamaTokenizer(): TokenizerLike {
  const mod = llamaTokenizerModule as unknown as TokenizerLike & {
    default?: TokenizerLike;
  };
  ...
  // Surface a descriptive error instead of the confusing "not a function".
  throw new Error("llama tokenizer module did not expose an encode() function");
}

const llamaTokenizer = resolveLlamaTokenizer();

interface Encoding {
  encode(text: string, ...rest: any[]): number[];
...
function asyncEncoderForModel(modelName: string): AsyncEncoder {
  ...
  if (!modelType || modelType === "none") {
    ...
    // Cannot find package 'js-tiktoken' imported from (upstream dev machine path)out/tiktokenWorkerPool.mjs
...
function encodingForModel(modelName: string): Encoding {
  ...
  // 旧实现在模型类型未知时回落到 js-tiktoken 的 gpt-4 编码。js-tiktoken 会往
  // bundle 里塞 5MB 的 BPE 词表，而本地 llama tokenizer 已经覆盖主路径
  // (countTokens 的默认 modelName 就是 llama2)，这里统一使用 llama：
  // 代价只是未知模型的 token 计数略有偏差，而计数本身即为估算值。
  if (!modelType || modelType === "none") {
