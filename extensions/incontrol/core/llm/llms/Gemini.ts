import { streamResponse } from "@incontrol/fetch";
...
import {
  ...
  convertIncontrolToolToGeminiFunction,
...
class Gemini extends BaseLLM {
  ...
  incontrolPartToGeminiPart(part: MessagePart): GeminiChatContentPart {
    if (part.type === "text") {
      return {
        text: part.text,
      };
            ...
            const assistantMsg: GeminiChatContent = {
              ...
              parts:
                typeof msg.content === "string"
                  ? [{ text: msg.content }]
                  : msg.content.map(this.incontrolPartToGeminiPart),
            };
    ...
    if (!isV1API) {
      ...
      if (options.tools?.length) {
        ...
        options.tools.forEach((tool) => {
          try {
            functions.push(convertIncontrolToolToGeminiFunction(tool));
          } catch (e) {
            console.warn(
