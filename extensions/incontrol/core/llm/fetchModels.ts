...
async function fetchProviderModelsViaListModels(
  ...
  try {
    ...
    // Unregistered provider names are treated as OpenAI-compatible gateways
    // when a custom apiBase is provided.
    if (!cls && !apiBase) {
      throw new Error(
        `Unknown LLM provider "${provider}" and no apiBase provided`,
      );
    }

    const llm = llmFromProviderAndOptions(
      cls ? provider : OpenAI.providerName,
      {
        apiKey,
