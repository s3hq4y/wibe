...
import {
  ...
  isIncontrolAgentConfigFile,
  isIncontrolConfigRelatedUri,
  ...
  private registerMessageHandlers(ideSettingsPromise: Promise<IdeSettings>) {
    ...
    on("config/deleteRule", async (msg) => {
      try {
        ...
        if (
          ...
          !isIncontrolConfigRelatedUri(filepath)
        ) {
    ...
    on("skills/create", async (msg) => {
      try {
        ...
        walkDirCache.invalidate();
        await this.configHandler.reloadConfig(
          "Skill created (skills/create message)",
        );
      } catch (error) {
    ...
    on("skills/write", async (msg) => {
      try {
        if (!isSkillFileUri(msg.data.path)) {
          throw new Error(
            "Only SKILL.md files inside a skills directory can be written",
          );
        }
        await this.ide.writeFile(msg.data.path, msg.data.content);
        walkDirCache.invalidate();
        await this.configHandler.reloadConfig(
          "Skill updated (skills/write message)",
        );
      } catch (error) {
    ...
    on("skills/delete", async (msg) => {
      try {
        if (!isSkillFileUri(msg.data.path)) {
          throw new Error(
            "Only SKILL.md files inside a skills directory can be deleted",
          );
        }
        const fileExists = await this.ide.fileExists(msg.data.path);
        if (fileExists) {
          await this.ide.removeFile(msg.data.path);
          markBundledSkillDeleted(msg.data.path);
          walkDirCache.invalidate();
          await this.configHandler.reloadConfig(
            "Skill deleted (skills/delete message)",
          );
        }
      } catch (error) {
    ...
    on("skills/setDisabled", async (msg) => {
      setSkillDisabled(msg.data.name, msg.data.disabled);
      await this.configHandler.reloadConfig(
        "Skill availability changed (skills/setDisabled message)",
      );
    });
    ...
    on("index/forceReIndex", async () => {
      // The codebase indexer has been removed; only the directory-walk cache
      // (used by @file / @folder submenus) needs to be refreshed.
      walkDirCache.invalidate();
      this.messenger.send("refreshSubmenuItems", {
        providers: "all",
      });
    ...
    on("files/created", async ({ data }) => {
      ...
      if (nonColocatedRuleUris.some(isIncontrolAgentConfigFile)) {
        await this.configHandler.refreshAll("Local config file created");
      } else if (nonColocatedRuleUris.some(isIncontrolConfigRelatedUri)) {
        await this.configHandler.reloadConfig(
          "incontrol config-related file created",
        );
      }
    });
    ...
    on("files/deleted", async ({ data }) => {
      ...
      if (nonColocatedRuleUris.some(isIncontrolAgentConfigFile)) {
        ...
        await this.configHandler.reloadConfig(
          "incontrol config-related file deleted",
        );
      }
    });
    ...
    on("tools/preprocessArgs", async ({ data: { toolName, args } }) => {
      ...
      try {
        ...
        let errorReason =
          e instanceof IncontrolError ? e.reason : IncontrolErrorReason.Unknown;
        let errorMessage =
          e instanceof Error
            ? e.message
  ...
  private async handleFilesChanged({
    ...
    if (data?.uris?.length) {
      ...
      for (const uri of data.uris) {
        ...
        } else if (isIncontrolConfigRelatedUri(uri)) {
          await this.configHandler.reloadConfig(
            "Local config-related file updated",
          );
        } else if (
          uri.endsWith(".incontrolignore") ||
          uri.endsWith(".gitignore")
        ) {
          // Ignore rules changed - drop the cached directory walks
