...
import { IDE } from "../..";
import {
  getGlobalIncontrolIgArray,
  getWorkspaceIncontrolIgArray,
} from "../../indexing/incontrolignore";
...
export async function shouldPrefilter(
  ...
  // Check whether we're in the incontrol config.json file
  ...
  const disableInFiles = [
    ...
    "*.prompt",
    ...getGlobalIncontrolIgArray(),
    ...(await getWorkspaceIncontrolIgArray(ide)),
  ];
