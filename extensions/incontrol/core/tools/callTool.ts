...
// Only the terminal tool has a core-side implementation; the edit tool is
// handled on the client (see gui/src/util/clientTools/callClientTool.ts).
...
export async function callTool(
  ...
  try {
    ...
    let errorMessage = `${e}`;
    let errorReason: IncontrolErrorReason | undefined = undefined;

    if (e instanceof IncontrolError) {
