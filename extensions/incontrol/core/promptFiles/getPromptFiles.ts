...
import { getIncontrolGlobalPath, readAllGlobalPromptFiles } from "../util/paths";
...
export async function getAllPromptFiles(
  ...
  // Also read from ~/.incontrol/prompts and ~/.incontrol/rules
  promptFiles.push(...readAllGlobalPromptFiles());

  const promptFilesFromRulesDirectory = readAllGlobalPromptFiles(
    path.join(getIncontrolGlobalPath(), RULES_DIR_NAME),
  );
