...
import { convertYamlMcpConfigToInternalMcpOptions } from "../../../config/yaml/yamlToIncontrolConfig";
import {
  DEFAULT_IGNORE_DIRS,
...
/**
 * Loads MCP configs from JSON files in ~/.incontrol/mcpServers and workspace .incontrol/mcpServers
 */
export async function loadJsonMcpConfigs(
  ...
  const mcpDirs = workspaceDirs.map((dir) =>
    joinPathsToUri(dir, WORKSPACE_CONFIG_DIR_NAME, "mcpServers"),
  );
  if (includeGlobal) {
