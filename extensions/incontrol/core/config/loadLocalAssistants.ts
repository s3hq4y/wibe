import { BLOCK_TYPES } from "@incontrol/config-yaml";
...
import { SYSTEM_PROMPT_DOT_FILE } from "./getWorkspaceIncontrolRuleDotFiles";
import { SUPPORTED_AGENT_FILES } from "./markdown";
export function isIncontrolConfigRelatedUri(uri: string): boolean {
  return (
    ...
    ((uri.includes(LEGACY_WORKSPACE_CONFIG_DIR_NAME) ||
      uri.includes(WORKSPACE_CONFIG_DIR_NAME)) &&
      (uri.endsWith(".yaml") ||
        uri.endsWith(".yml") ||
        uri.endsWith(".json"))) ||
    [...BLOCK_TYPES, "agents", "assistants", "configs"].some((blockType) =>
      uri.includes(`${LEGACY_WORKSPACE_CONFIG_DIR_NAME}/${blockType}`) ||
        uri.includes(`${WORKSPACE_CONFIG_DIR_NAME}/${blockType}`),
    )
  );
}

export function isIncontrolAgentConfigFile(uri: string): boolean {
  ...
  return (
    ["agents", "assistants", "configs"].some(
      (sub) =>
        normalizedUri.includes(`/${LEGACY_WORKSPACE_CONFIG_DIR_NAME}/${sub}/`) ||
        normalizedUri.includes(`/${WORKSPACE_CONFIG_DIR_NAME}/${sub}/`),
    )
  );
}
...
export function getDotIncontrolSubDirs(
  ide: IDE,
  ...
  // Workspace <configDir>/<subDirName> (legacy dir still read for compatibility)
  if (options.includeWorkspace) {
    fullDirs = workspaceDirs.flatMap((dir) => [
      joinPathsToUri(dir, WORKSPACE_CONFIG_DIR_NAME, subDirName),
      joinPathsToUri(dir, LEGACY_WORKSPACE_CONFIG_DIR_NAME, subDirName),
    ]);
  }

  // ~/.incontrol/<subDirName>
...
/**
 * This method searches in both ~/.incontrol and workspace .incontrol
 * for all YAML/Markdown files in the specified subdirectory, for example .continue/assistants or .continue/prompts
 */
export async function getAllDotIncontrolDefinitionFiles(
  ide: IDE,
  options: LoadAssistantFilesOptions,
  subDirName: string,
): Promise<{ path: string; content: string }[]> {
  ...
  const fullDirs = getDotIncontrolSubDirs(
    ide,
    options,
    workspaceDirs,
    subDirName,
