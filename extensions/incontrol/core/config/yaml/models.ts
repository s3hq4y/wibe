...
import { IncontrolConfig, ILLMLogger, LLMOptions } from "../..";
...
function getModelClass(
  ...
  // Unregistered provider names (e.g. "chatgpt", "volcengine" proxies) are
  // treated as OpenAI-compatible gateways when a custom apiBase is provided,
  // so `model: AUTODETECT` can still list models via `{apiBase}/models`.
  if (model.apiBase) {
    console.warn(
      `Unknown provider "${model.provider}", falling back to the OpenAI-compatible API at ${model.apiBase}`,
    );
    return OpenAI;
  }
