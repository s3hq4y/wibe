...
import { IncontrolConfig, ILLMLogger } from "../..";
...
describe("llmsFromModelConfig requestOptions merging", () => {
  ...
  let mockConfig: IncontrolConfig;
  ...
  it("should fall back to OpenAI-compatible API for unknown provider with apiBase", async () => {
    const model: ModelConfig = {
      name: "volcengine-model",
      provider: "volcengine",
      model: "doubao-pro-32k",
      apiBase: "https://ark.example.com/api/v3",
    };
    ...
    // Unknown provider + apiBase falls back to the OpenAI-compatible class
    expect(result).toHaveLength(1);
    expect(result[0].model).toBe("doubao-pro-32k");
    expect(result[0].apiBase).toBe("https://ark.example.com/api/v3/");
  });
