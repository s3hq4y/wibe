...
import {
  ...
  convertYamlRuleToIncontrolRule,
} from "./yamlToIncontrolConfig";
...
async function loadConfigYaml(options: {
  ...
  const localBlockPromises = BLOCK_TYPES.map(async (blockType) => {
    const localBlocks = await getAllDotIncontrolDefinitionFiles(
      ide,
...
export async function configYamlToIncontrolConfig(options: {
  unrolledAssistant: AssistantUnrolled;
  ide: IDE;
  ideInfo: IdeInfo;
  uniqueId: string;
  ...
  for (const rule of config.rules ?? []) {
    const convertedRule = convertYamlRuleToIncontrolRule(rule);
    incontrolConfig.rules.push(convertedRule);
  }

  incontrolConfig.data = config.data?.map((d) => ({
    ...d,
    requestOptions: mergeConfigYamlRequestOptions(
      d.requestOptions,
      incontrolConfig.requestOptions,
    ),
  }));
  incontrolConfig.docs = config.docs?.map((doc) => ({
    title: doc.name,
  ...
  try {
    ...
    promptFiles.forEach((file) => {
      try {
        ...
        if (slashCommand) {
          incontrolConfig.slashCommands?.push(slashCommand);
        }
      } catch (e) {
  ...
  for (const model of config.models ?? []) {
    ...
    try {
      ...
      if (model.roles?.includes("chat")) {
        incontrolConfig.modelsByRole.chat.push(...llms);
      }

      if (model.roles?.includes("summarize")) {
        incontrolConfig.modelsByRole.summarize.push(...llms);
      }

      if (model.roles?.includes("apply")) {
        incontrolConfig.modelsByRole.apply.push(...llms);
      }

      if (model.roles?.includes("edit")) {
        incontrolConfig.modelsByRole.edit.push(...llms);
      }

      if (model.roles?.includes("autocomplete")) {
        incontrolConfig.modelsByRole.autocomplete.push(...llms);
      }
      ...
      if (model.roles?.includes("embed")) {
        if (model.provider === "transformers.js") {
          localErrors.push({
            fatal: false,
            message: `The built-in transformers.js embeddings provider has been removed; model "${model.name}" will be ignored.`,
          });
        } else {
          incontrolConfig.modelsByRole.embed.push(...llms);
        }
      }

      if (model.roles?.includes("rerank")) {
        incontrolConfig.modelsByRole.rerank.push(...llms);
      }

      if (model.roles?.includes("subagent")) {
        incontrolConfig.modelsByRole.subagent.push(...llms);
      }
    } catch (e) {
      localErrors.push({
        fatal: false,
  ...
  incontrolConfig.contextProviders = providers;
  ...
  return { config: incontrolConfig, errors: localErrors };
}

export async function loadIncontrolConfigFromYaml(options: {
  ide: IDE;
  ideSettings: IdeSettings;
  ideInfo: IdeInfo;
  uniqueId: string;
  ...
  const { config: incontrolConfig, errors: localErrors } =
    await configYamlToIncontrolConfig({
  ...
  // See core/config/load.ts: this fork defaults telemetry to off.
