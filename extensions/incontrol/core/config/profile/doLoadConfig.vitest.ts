...
import type { PackageIdentifier } from "@incontrol/config-yaml";
...
vi.mock("../yaml/loadYaml", () => ({
  loadIncontrolConfigFromYaml: (...args: any[]) => mockLoadYaml(...args),
}));
vi.mock("../load", () => ({
  loadIncontrolConfigFromJson: (...args: any[]) => mockLoadJson(...args),
}));
vi.mock("../migrateSharedConfig", () => ({
  migrateJsonSharedConfig: vi.fn(),
}));
vi.mock("../getWorkspaceIncontrolRuleDotFiles", () => ({
  getWorkspaceIncontrolRuleDotFiles: vi
    .fn()
