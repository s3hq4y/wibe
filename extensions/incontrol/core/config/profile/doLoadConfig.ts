...
import { goalSlashCommand } from "../../promptFiles/goalPrompt";
import { finalizeTools, getConfigDependentToolDefinitions } from "../../tools";
...
import { getWorkspaceIncontrolRuleDotFiles } from "../getWorkspaceIncontrolRuleDotFiles";
import { loadIncontrolConfigFromJson } from "../load";
...
import { loadIncontrolConfigFromYaml } from "../yaml/loadYaml";

async function loadRules(ide: IDE) {
  const rules: RuleWithSource[] = [];
  const errors = [];

  // Add rules from .incontrolrules files
  const { rules: yamlRules, errors: incontrolRulesErrors } =
    await getWorkspaceIncontrolRuleDotFiles(ide);
  rules.unshift(...yamlRules);
  errors.push(...incontrolRulesErrors);

  // Add rules from markdown files in .incontrol/rules
...
export default async function doLoadConfig(options: {
  ...
  overrideConfigJson?: SerializedIncontrolConfig;
  ...
  let newConfig: IncontrolConfig | undefined;
  ...
  if (
    ...
    const result = await loadIncontrolConfigFromYaml({
      ide,
      ideSettings,
      ideInfo,
      uniqueId,
    ...
    const result = await loadIncontrolConfigFromJson(
      ide,
      ideSettings,
      ideInfo,
      uniqueId,
  ...
  // Built-in /goal command. This is the profile-based loader that actually
  // feeds the GUI (the legacy loader in core/config/load.ts does not), so the
  // command has to be registered here to show up in the "/" dropdown.
  newConfig.slashCommands.push(goalSlashCommand);
  ...
  if (newConfig.experimental?.slimToolDescriptions) {
    newConfig.tools = finalizeTools(newConfig.tools, true);
  }
