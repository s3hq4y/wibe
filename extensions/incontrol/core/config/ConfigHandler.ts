import { ConfigResult, ConfigValidationError } from "@incontrol/config-yaml";
...
import {
  getAllDotIncontrolDefinitionFiles,
...
type ConfigUpdateFunction = (payload: ConfigResult<IncontrolConfig>) => void;
...
export class ConfigHandler {
  ...
  async getLocalProfiles(options: LoadAssistantFilesOptions) {
    /**
     * Users can define as many local agents as they want in a `.incontrol/agents` (or legacy .continue/agents) folder
     */
    ...
    if (options.includeWorkspace) {
      ...
      const allFiles = (
        await Promise.all([
          getAllDotIncontrolDefinitionFiles(this.ide, yamlOptions, "assistants"),
          getAllDotIncontrolDefinitionFiles(this.ide, yamlOptions, "agents"),
          getAllDotIncontrolDefinitionFiles(this.ide, yamlOptions, "configs"),
        ])
      ).flat();
  ...
  private notifyConfigListeners(result: ConfigResult<IncontrolConfig>) {
  ...
  async getSerializedConfig(): Promise<
    ConfigResult<BrowserSerializedIncontrolConfig>
  > {
  ...
  async loadConfig(): Promise<ConfigResult<IncontrolConfig>> {
    await this.isInitialized;
    if (!this.currentProfile) {
      return {
        config: undefined,
