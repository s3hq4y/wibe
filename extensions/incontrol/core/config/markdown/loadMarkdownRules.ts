...
/**
 * Loads rules from markdown files in the .incontrol/rules and .incontrol/prompts directories
...
export async function loadMarkdownRules(ide: IDE): Promise<{
  ...
  // Load markdown files from both .incontrol/rules and .incontrol/prompts
  const dirsToCheck = [RULES_DIR_NAME, PROMPTS_DIR_NAME];

  for (const dirName of dirsToCheck) {
    try {
      const markdownFiles = await getAllDotIncontrolDefinitionFiles(
        ide,
        {
          includeGlobal: true,
