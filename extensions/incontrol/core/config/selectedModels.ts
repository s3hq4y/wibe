...
import { IncontrolConfig, ILLM } from "..";
...
export function rectifySelectedModelsFromGlobalContext(
  incontrolConfig: IncontrolConfig,
  profileId: string,
): IncontrolConfig {
  ...
  for (const role of roles) {
    ...
    if (currentSelection) {
      const match = incontrolConfig.modelsByRole[role].find(
    ...
    if (!newModel && incontrolConfig.modelsByRole[role].length > 0) {
      newModel = incontrolConfig.modelsByRole[role][0];
    }
