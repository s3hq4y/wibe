...
export function resolveSerializedConfig(
  ...
  const config = JSONC.parse(content) as unknown as SerializedIncontrolConfig;
  ...
  return JSONC.parse(content) as unknown as SerializedIncontrolConfig;
}
...
function loadSerializedConfig(
  workspaceConfigs: IncontrolRcJson[],
  ideSettings: IdeSettings,
  ideType: IdeType,
  overrideConfigJson: SerializedIncontrolConfig | undefined,
  ide: IDE,
): ConfigResult<SerializedIncontrolConfig> {
  let config: SerializedIncontrolConfig = overrideConfigJson!;
  if (!config) {
    try {
  ...
  // No upstream hub/telemetry endpoint ships with this fork, so telemetry is
...
async function serializedToIntermediateConfig(
  initial: SerializedIncontrolConfig,
  ide: IDE,
): Promise<Config> {
...
async function intermediateToFinalConfig({
...
}): Promise<{ config: IncontrolConfig; errors: ConfigValidationError[] }> {
  ...
  const incontrolConfig: IncontrolConfig = {
    ...config,
    contextProviders,
    tools: finalizeTools(
      getBaseToolDefinitions(),
      !!config.experimental?.slimToolDescriptions,
    ),
    mcpServerStatuses: [],
    slashCommands: [],
    modelsByRole: {
  ...
  for (const cmd of config.slashCommands ?? []) {
    if ("source" in cmd) {
      incontrolConfig.slashCommands.push(cmd);
    } else {
      incontrolConfig.slashCommands.push({
        ...cmd,
  ...
  const inlineEditModel = getModelByRole(incontrolConfig, "inlineEdit")?.title;
  if (inlineEditModel) {
    const match = incontrolConfig.modelsByRole.chat.find(
      (m) => m.title === inlineEditModel,
    );
    if (match) {
      incontrolConfig.selectedModelByRole.edit = match;
      incontrolConfig.modelsByRole.edit = [match]; // The only option if inlineEdit role is set
    } else {
      errors.push({
        fatal: false,
  ...
  if (applyBlockModel) {
    ...
    if (match) {
      incontrolConfig.selectedModelByRole.apply = match;
      incontrolConfig.modelsByRole.apply = [match]; // The only option if applyCodeBlock role is set
    } else {
      errors.push({
        fatal: false,
  ...
  return { config: incontrolConfig, errors };
}
...
async function finalToBrowserConfig(
  final: IncontrolConfig,
  ide: IDE,
): Promise<BrowserSerializedIncontrolConfig> {
  return {
  ...
  try {
    ...
    try {
      ...
      await ide.showToast(
        "error",
        [
          "config.ts has been deprecated and esbuild is no longer automatically installed by incontrol.",
...
async function tryBuildConfigTs() {
  try {
    ...
    console.log(
      `Build error. Please check your ~/.incontrol/config.ts file: ${e}`,
    );
  }
}
...
async function loadIncontrolConfigFromJson(
  ide: IDE,
  ideSettings: IdeSettings,
  ideInfo: IdeInfo,
  uniqueId: string,
...
export {
  ...
  loadIncontrolConfigFromJson,
  type BrowserSerializedIncontrolConfig,
};
