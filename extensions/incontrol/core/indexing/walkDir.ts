...
import { getGlobalIncontrolIgArray } from "./incontrolignore";
...
export async function getIgnoreContext(
  ...
  // These are done separately so that .incontrolignore can override .gitignore
  const gitIgnoreFile = dirFiles.find((name) => name === ".gitignore");
  const incontrolIgnoreFile = dirFiles.find(
    (name) => name === ".incontrolignore" || name === ".incontrolignore",
  );
  ...
  const getIncontrolIgnorePatterns = async () => {
    if (incontrolIgnoreFile) {
      const contents = await ide.readFile(`${currentDir}/${incontrolIgnoreFile}`);
      return gitIgArrayFromFile(contents);
    }
    return [];
  };
  ...
  const ignoreContext = ignore()
    ...
    .add(defaultAndGlobalIgnores) // default file/folder ignores followed by global .incontrolignore - this is combined for speed
    .add(ignoreArrays[1]); // local .incontrolignore
