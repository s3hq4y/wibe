...
export interface IdeSettings {
  ...
  userToken: string;
  incontrolTestEnvironment: "none" | "production" | "staging" | "local";
}
...
export interface SlashCommand extends SlashCommandDescription {
  run: (sdk: IncontrolSDK) => AsyncGenerator<string | undefined>;
}

export interface SlashCommandWithSource extends SlashCommandDescription {
  run?: (sdk: IncontrolSDK) => AsyncGenerator<string | undefined>; // Optional - only needed for legacy
...
export interface IncontrolUIConfig {
  ...
  /** A user-authored prompt enabled explicitly from the chat toolbar. */
  manualSystemMessage?: string;
}
...
export interface ExperimentalConfig {
  ...
  /**
   * If true, tool descriptions in the tools array / system message are replaced
   * by one-line summaries, and a get_tool_usage tool is added for on-demand
   * full documentation.
   */
  ...
  /**
     * How many times the agent may re-send the session goal when the model ends
     * a turn without calling a tool. 0 disables the auto-continue behaviour.
...
// config.json
export interface SerializedIncontrolConfig {
  env?: string[];
...
export type IncontrolRcJson = Partial<SerializedIncontrolConfig> & {
  mergeBehavior: ConfigMergeType;
};

// config.ts - give users simplified interfaces
export interface Config {
  /** If set to true, incontrol will collect anonymous usage data to improve the product. If set to false, we will collect nothing. Read here to learn more: https://docs.continue.dev/telemetry */
  allowAnonymousTelemetry?: boolean;
  ...
  disableSessionTitles?: boolean;
  /** An optional token to identify a user. Not used by incontrol unless you write custom coniguration that requires such a token */
  userToken?: string;
...
export interface IncontrolConfig {
  allowAnonymousTelemetry?: boolean;
  // systemMessage?: string;
  completionOptions?: BaseCompletionOptions;
  requestOptions?: RequestOptions;
...
export interface BrowserSerializedIncontrolConfig {
  allowAnonymousTelemetry?: boolean;
  // systemMessage?: string;
  completionOptions?: BaseCompletionOptions;
  requestOptions?: RequestOptions;
...
export interface Skill {
  ...
  files: string[];
  license?: string;
  /** Actual file URI of the SKILL.md file */
  fileUri?: string;
  /** Raw file content including frontmatter, for editing */
  rawContent?: string;
  /** Disabled skills are hidden from the read_skill tool */
  disabled?: boolean;
}
