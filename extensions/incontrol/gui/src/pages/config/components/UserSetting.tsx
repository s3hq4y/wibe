...
interface TextareaUserSettingProps extends BaseUserSettingProps {
  type: "textarea";
  value: string;
  onChange: (value: string) => void;
  placeholder?: string;
  ...
  isValid: boolean;
  rows?: number;
  /** Small text button under the box, e.g. "Insert template". */
          ...
          >
            <div className="flex items-start gap-2">
              <textarea
                ...
                rows={props.rows ?? 8}
                disabled={disabled}
                className={`text-vsc-foreground border-command-border bg-vsc-input-background focus:border-border-focus focus:ring-border-focus min-h-32 w-full resize-y rounded-md border border-solid px-2 py-1.5 font-mono text-xs outline-none focus:ring-1 ${
                  props.isDirty
                    ? props.isValid
            ...
            {props.action && (
              <button
                ...
                className="text-description hover:text-foreground mt-1 cursor-pointer border-none bg-transparent p-0 text-[11px] underline underline-offset-2"
              >
                {props.action.label}
              </button>
            )}
