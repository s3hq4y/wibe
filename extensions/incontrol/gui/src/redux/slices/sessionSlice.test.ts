...
describe("sessionSlice streamUpdate", () => {
  ...
  describe("newSession goal reset", () => {
    it("should clear the goal when starting a new session", () => {
      const initialState = createInitialState();
      const withGoal = sessionSlice.reducer(
        initialState,
        sessionSlice.actions.setGoal("Finish the refactor"),
      );
      expect(withGoal.goal?.text).toBe("Finish the refactor");
      ...
      expect(newState.goal).toBeUndefined();
    });

    it("should clear the goal when loading a previous session", () => {
      const initialState = createInitialState();
      const withGoal = sessionSlice.reducer(
        initialState,
        sessionSlice.actions.setGoal("Old unfinished goal"),
      );
      ...
      expect(newState.goal).toBeUndefined();
      expect(newState.id).toBe("old-session");
    });
  });
});
