...
import { BrowserSerializedIncontrolConfig } from "core";
import { DEFAULT_CONTEXT_LENGTH } from "core/llm/constants";

export type ConfigState = {
  configError: ConfigValidationError[] | undefined;
  config: BrowserSerializedIncontrolConfig;
  loading: boolean;
};

export const EMPTY_CONFIG: BrowserSerializedIncontrolConfig = {
  slashCommands: [],
  contextProviders: [],
  tools: [],
  mcpServerStatuses: [],
...
export const configSlice = createSlice({
  ...
  reducers: {
    setConfigResult: (
      ...
      {
        payload: result,
      }: PayloadAction<ConfigResult<BrowserSerializedIncontrolConfig>>,
    ) => {
    ...
    updateConfig: (
      state,
      { payload: config }: PayloadAction<BrowserSerializedIncontrolConfig>,
    ) => {
      state.config = config;
    },
