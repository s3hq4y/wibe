...
type SessionState = {
  ...
  /** True after the user explicitly enables the configured prompt for this session. */
  manualSystemMessageEnabled?: boolean;
  isPruned?: boolean;
  ...
  /**
   * Goal the user set for this session. When present and not completed, the
   * agent loop nudges the model instead of ending the turn. Only kept in
   * memory - it is intentionally not persisted, so a reload starts clean
   * rather than resuming an unattended run.
   */
  goal?: {
    text: string;
    nudgeCount: number;
...
export const INITIAL_SESSION_STATE: SessionState = {
  ...
  manualSystemMessageEnabled: false,
  compactionLoading: {},
};
...
export const sessionSlice = createSlice({
  ...
  reducers: {
    ...
    newSession: (state, { payload }: PayloadAction<Session | undefined>) => {
      ...
      state.isStreaming = false;
      state.manualSystemMessageEnabled = false;
      state.symbols = {};
      ...
      // The goal belongs to the previous conversation - never carry it over,
      // whether starting a fresh session or loading an old one.
    ...
    incrementGoalNudge: (state) => {
      if (state.goal) {
        state.goal.nudgeCount += 1;
      }
    },
    completeGoal: (state) => {
      if (state.goal) {
    ...
    setManualSystemMessageEnabled: (
      ...
      state.manualSystemMessageEnabled = action.payload;
    },
