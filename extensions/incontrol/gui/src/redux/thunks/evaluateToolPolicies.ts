...
async function evaluateToolPolicy(
  ...
  // An explicit user choice in the tool settings wins over the dynamic
  // security evaluation. Otherwise the terminal security layer can silently
  // downgrade "Automatic" back to "Ask First" (the clamp below only stops the
  // dynamic policy from being MORE lenient than the base, never from being
  // more restrictive), which made the setting feel like it did nothing.
  const explicitUserPolicy = toolPolicies[toolCallState.toolCall.function.name];
  if (explicitUserPolicy === "allowedWithoutPermission") {
    return { policy: "allowedWithoutPermission", toolCallState };
  }
