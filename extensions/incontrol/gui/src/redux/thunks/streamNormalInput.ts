...
import {
  MANUAL_SYSTEM_PROMPT_TEMPLATE,
  stripToolGuidance,
} from "../../util/systemPromptTemplate";
...
>(
  ...
  async (
    ...
    // The configured system prompt is deliberately opt-in and one-shot.
    // The toolbar explicitly arms it for the next outbound chat request only.
    const shouldSendManualSystemMessage =
      state.session.manualSystemMessageEnabled;
    // An armed but unfilled prompt falls back to the built-in tool template, so
    // the mode is usable straight away without writing a prompt first.
    const manualSystemMessage =
      state.config.config.ui?.manualSystemMessage?.trim() ?? "";
    const baseSystemMessage = shouldSendManualSystemMessage
      ? manualSystemMessage ||
        (activeTools.length > 0
          ? MANUAL_SYSTEM_PROMPT_TEMPLATE
          : stripToolGuidance(MANUAL_SYSTEM_PROMPT_TEMPLATE))
      : "";

    // Consume the explicit manual-send action before starting this request, so it
    // cannot be injected automatically into later requests in the same session.
    if (shouldSendManualSystemMessage) {
      dispatch(setManualSystemMessageEnabled(false));
    }
    ...
    const systemMessage = systemToolsFramework
      ? addSystemMessageToolsToSystemMessage(
          ...
          baseSystemMessage,
          activeTools,
          !!state.config.config.experimental?.slimToolDescriptions,
        )
      : baseSystemMessage;
    ...
    if (originalToolCalls.length === 0) {
      // Goal tracking: if a goal is set and the model ended its turn without
      // calling a tool, nudge it instead of wrapping up. The nudge count is a
      // hard brake - production has no depth guard (the one above only fires
      // under NODE_ENV=test), so without it a model that keeps replying in
      // plain text would loop forever and burn tokens.
      const { goal } = getState().session;
      const maxGoalNudges =
        getState().config.config.experimental?.maxGoalNudges ?? 5;

      if (
        goal &&
        !goal.completed &&
        maxGoalNudges > 0 &&
        goal.nudgeCount < maxGoalNudges
      ) {
        dispatch(incrementGoalNudge());
        const defaultNudgeMessage = `Your goal is: {{goal}}
...
Simply stating that the goal is complete in plain text does NOT stop goal tracking - only the mark_goal_complete tool call does.`;
        ...
        const configuredNudgeMessage =
          getState().config.config.experimental?.goalNudgeMessage;
        const content = (configuredNudgeMessage ?? defaultNudgeMessage).replace(
          "{{goal}}",
          goal.text,
        );
