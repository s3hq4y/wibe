...
export async function resolveEditorContent({
  ...
  // Built-in /goal: store the input as the session goal. The rendered text is
  // still returned below as the user message, so the agent starts on it at once.
  if (slashCommandName === "goal") {
    const goalText = stripImages(
      parts.filter((part) => part.type === "text"),
    ).trim();
    if (goalText) {
      dispatch(setGoal(goalText));
    }
  }
