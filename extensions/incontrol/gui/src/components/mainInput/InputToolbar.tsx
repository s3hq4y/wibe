...
function InputToolbar(props: InputToolbarProps) {
  ...
  const manualSystemMessage = useAppSelector(
    (store) => store.config.config.ui?.manualSystemMessage?.trim() ?? ""
  );
  const manualSystemMessageEnabled = useAppSelector(
    (store) => store.session.manualSystemMessageEnabled
  );
  ...
  return (
    <>
      <div
        ...
        className={`find-widget-skip bg-vsc-input-background flex min-w-0 select-none flex-row flex-wrap items-center justify-between gap-1 pt-1 ${
          props.hidden
      ...
      >
        <div className="xs:gap-1.5 flex min-w-0 flex-row flex-wrap items-center gap-1">
          ...
          {!isInEdit && (
            <ToolTip
              place="top"
              content={
                manualSystemMessageEnabled
                  ? "System prompt will be sent with your next message (click to cancel)"
                  : manualSystemMessage
                  ? "Send your saved system prompt with your next message"
                  : "Send the built-in tool template with your next message (no prompt saved yet)"
              }
            >
              <HoverItem
                onClick={() =>
                  dispatch(
                    setManualSystemMessageEnabled(!manualSystemMessageEnabled)
                  )
                }
              >
                <DocumentTextIcon
                  className={`h-3 w-3 hover:brightness-125 ${
                    manualSystemMessageEnabled ? "text-foreground" : ""
                  }`}
                />
              </HoverItem>
            </ToolTip>
        ...
        <div
          className="text-description flex shrink-0 items-center gap-2 whitespace-nowrap"
          style={{
            fontSize: tinyFont,
          }}
        >
