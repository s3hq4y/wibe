...
export const SecureImageComponent: React.FC<SecureImageComponentProps> = ({
  ...
  return (
    <ImagePlaceholder>
      <WarningText>
        {t("Image blocked for security. External images can leak data through URL parameters. Click to load if you trust the source.")}</WarningText>
      ...
      {hasQueryParams && (
        <QueryParamsDisplay>
          <strong>{t("Warning: URL contains query parameters:")}</strong>
      ...
      {imageError && (
        <div style={{ color: lightGray, fontSize: "12px", marginTop: "8px" }}>
          {t("Failed to load image. The URL may be invalid or inaccessible.")}</div>
      )}
